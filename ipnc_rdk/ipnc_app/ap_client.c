...
int airkiss_replayRandom(unsigned char random)
{
    char rgMessage[128];
    int iFd;
    int iSendbytes;
    ...
    if (setsockopt(iFd, SOL_SOCKET, SO_BROADCAST, &iOptval, sizeof(int)) < 0)
    {
	...
	sprintf(rgMessage, "%x",random);
   	for(i = 0; i < 6; i ++)
    {
        if ((iSendbytes = sendto(iFd, &random, sizeof(random), 0, (struct sockaddr*)&Addr, sizeof(struct sockaddr))) == -1)
        {
